...
export const ProjectParticipants = ({ projectId, participants, setProject, onDeleteProject}) => {
  ...
  const fetchUsers = debounce(async (input) => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        console.warn("Очікувався масив, отримано:", data);
        setOptions([]);
      }
    } catch (err) {
  ...
  const addUser = async () => {
    ...
    if (res.ok) {
      // не додавати, якщо вже є локально
      setProject((prev) => {
        ...
        return {
          ...prev,
          participants: [...prev.participants, { user: selectedUser._id, role: "user" }],
        };
      });
      setSelectedUser(null);
      setOptions([]);
      setMessage("✅ Користувач доданий!");
    } else {
  ...
  const deleteProject = async () => {
    ...
    onDeleteProject(); // callback из родителя (например, навигация назад)
  };
